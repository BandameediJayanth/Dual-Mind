...
export class MLInference {
    ...
    createRuleBasedModel() {
        return {
            predict: (features) => {
                ...
                // Calculate decision speed score (faster is better, up to a point)
                const avgTime = features.avgDecisionTime || 2000;
                const decisionSpeed = avgTime < 500 ? 0.5 : // Too fast might be random
                    avgTime < 1000 ? 1.0 :
                    avgTime < 2000 ? 0.9 :
                    avgTime < 4000 ? 0.7 :
                    avgTime < 6000 ? 0.5 : 0.3;
                ...
                score += (features.patternSuccessRate || 0.5) * weights.patternSuccessRate * 100;
                score += (features.consistencyScore || 0.5) * weights.consistencyScore * 100;
                score += (features.optimalPlayRate || 0) * weights.optimalPlayRate * 100;
                score += decisionSpeed * weights.decisionSpeed * 100;
                score += errorPenalty * weights.errorPenalty * 100;
                
                // Add game-specific bonuses
                if (features.strategicDepth) {
                    score += features.strategicDepth * 5;
                }
                if (features.memoryAccuracy) {
                    score += features.memoryAccuracy * 5;
                }
                if (features.logicalReasoning) {
                    score += features.logicalReasoning * 5;
                }
    ...
    async predict(features) {
        ...
        const result = {
            ...
            explanation: this.generateExplanation(features, prediction.score, skillTier)
        };
    ...
    normalizeFeatures(features) {
        ...
        Object.entries(this.featureScalers).forEach(([key, scaler]) => {
            if (features[key] !== undefined) {
                normalized[key] = (features[key] - scaler.min) / (scaler.max - scaler.min);
    ...
    calculateFeatureContributions(features) {
        ...
        const featureNames = {
            ...
            patternSuccessRate: 'Pattern Recognition',
        ...
        Object.entries(featureNames).forEach(([key, name]) => {
            if (features[key] !== undefined) {
                ...
                contributions.push({
                    ...
                    impact: value >= 0.6 ? 'positive' : value >= 0.4 ? 'neutral' : 'negative'
                });
            }
        });
