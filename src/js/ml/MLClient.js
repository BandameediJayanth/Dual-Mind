...
export class MLClient {
  ...
  async analyzeTrends(sessions) {
    ...
    if (this.pythonServiceAvailable) {
      try {
        const result = await this.callPythonAPI("/analyze/trends", {
          sessions,
        });
        result.source = "python_ml";
        return result;
  ...
  fallbackTrendAnalysis(sessions) {
    ...
    const scores = sessions.map(
      (s) => s.performanceIndex || s.performance_index || s.score || 50,
    );
