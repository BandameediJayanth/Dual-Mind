...
export class FourInARow {
  ...
  _bindEvents() {
    ...
    this.boardElement
      .querySelector("#fiar-reset")
      .addEventListener("click", () => this._resetGame());
  }
  ...
  _handleColClick(col) {
    ...
    const isWinningMove = this._wouldWinAt(row, col, this.currentPlayer);
    const isBlockingMove = !isWinningMove && this._wouldBlockAt(col, opponent);
    const isOptimal = isWinningMove || isBlockingMove;
  ...
  _updateTurn() {
    ...
    if (turnEl)
      turnEl.textContent =
        this.currentPlayer === "red" ? "Red's Turn" : "Blue's Turn";
  }

  _checkWin(row, col) {
    const player = this.board[row][col];
    return (
      this._checkDir(row, col, 0, 1, player) ||
  ...
  _highlightWin(row, col) {
    ...
    for (const [dr, dc] of dirs) {
      ...
      if (cells.length >= 4) {
        cells.forEach(([r, c]) => {
          const el = this.boardElement.querySelector(
  ...
  /** Returns true if placing `player` at (row,col) would win */
  _wouldWinAt(row, col, player) {
    this.board[row][col] = player;
    const wins = this._checkWin(row, col);
    this.board[row][col] = "";
    return wins;
  }

  /** Returns true if opponent would win if they played in this column */
  _wouldBlockAt(col, opponent) {
    const row = this._getLowestEmptyRow(col);
    if (row === -1) return false;
    return this._wouldWinAt(row, col, opponent);
  }

  _endGame(result) {
    this.gameActive = false;
    const winner =
      result === "win" ? (this.currentPlayer === "red" ? 1 : 2) : "draw";
    if (result === "win") {
      this.scores[this.currentPlayer]++;
      const scoreEl = this.boardElement.querySelector(
        `#fiar-score-${this.currentPlayer}`,
      );
    ...
    this.eventBus?.emit("game:end", {
      ...
      winReason:
        result === "win"
          ? `${this.currentPlayer} connected four!`
          : "Board is full",
  ...
  getState() {
    return {
      board: this.board,
      currentPlayer: this.currentPlayer === "red" ? 1 : 2,
