...
export class SeaWars {
    ...
    _initEngine() {
        ...
        let currentPlayer = 1, placingShips = true, currentShipIndex = 0, gameOver = false;
        ...
        function startGame() {
            ...
            gameOver = false; placingShips = true; currentShipIndex = 0; currentPlayer = 1;
            self.moveCount = 0; self.sessionStartTime = Date.now();
        ...
        function placeShip(index) {
            ...
            if (col + size > GRID_SIZE) { info.textContent = 'Not enough space! Try another position.'; return; }
            for (let i = 0; i < size; i++) {
                if (board[row * GRID_SIZE + col + i].ship) { info.textContent = 'Ships cannot overlap!'; return; }
            }
            for (let i = 0; i < size; i++) {
        ...
        function attackCell(playerNum, index) {
            ...
            if (board[index].hit) { info.textContent = 'Already attacked here!'; return; }
            ...
            if (board[index].ship) {
                ...
                if (checkWin(board)) {
                    ...
                    eventBus?.emit('game:end', {
                        ...
                        scores: { 1: currentPlayer === 1 ? 1 : 0, 2: currentPlayer === 2 ? 1 : 0 }
                    });
                }
            } else {
        ...
        function checkWin(board) { return board.every(cell => !cell.ship || cell.hit); }
    }
