...
export class MemoryMatch {
    ...
    _bindEvents() {
        ...
        this.boardElement.querySelector('#mm-new-game').addEventListener('click', () => this._newGame());
        this.boardElement.querySelector('#mm-reset-stats').addEventListener('click', () => {
            this.playerScores = { 1: 0, 2: 0 };
            this._updateDisplay();
        });
    }
        ...
        if (card1.dataset.symbol === card2.dataset.symbol) {
            setTimeout(() => {
                ...
                if (this.matchedPairs === this.symbols.length) {
                    ...
                    this._setStatus(`Player ${this.currentPlayer} found a match! Gets another turn! 🎉`);
                    this._updateDisplay();
                }
            }, 500);
        } else {
    ...
    _gameComplete() {
        this.gameStarted = false;
        const winner = this.playerScores[1] > this.playerScores[2] ? 1
                     : this.playerScores[2] > this.playerScores[1] ? 2 : 'draw';
    ...
    _updateDisplay() {
        ...
        if (turn) {
            turn.textContent = this.gameStarted && this.matchedPairs < this.symbols.length
                ? `Player ${this.currentPlayer}'s Turn`
                : this.matchedPairs >= this.symbols.length ? 'Game Over!' : 'Click a card to start!';
        }
        if (p1) p1.classList.toggle('active', this.currentPlayer === 1 && this.gameStarted);
        if (p2) p2.classList.toggle('active', this.currentPlayer === 2 && this.gameStarted);
    }

    getState() { return { currentPlayer: this.currentPlayer, gameActive: this.gameStarted }; }
