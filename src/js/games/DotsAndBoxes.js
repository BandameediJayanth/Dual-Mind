...
export class DotsAndBoxes {
  ...
  _initEngine() {
    ...
    const popupRestartBtn =
      this.boardElement.querySelector("#dab-popup-restart");
    ...
    function onLineClick(e) {
      ...
      eventBus?.emit("game:move", {
        ...
        position: {
          ...
          row: parseInt(el.dataset.r),
          col: parseInt(el.dataset.c),
        },
        timestamp: Date.now(),
        decisionTime: 0,
      });
    ...
    function showWinner() {
      ...
      if (scores[1] > scores[2]) {
        text = `Player 1 Wins! (${scores[1]} vs ${scores[2]})`;
        winner = 1;
      } else if (scores[2] > scores[1]) {
        text = `Player 2 Wins! (${scores[2]} vs ${scores[1]})`;
        winner = 2;
      } else {
        text = `It's a Draw! (${scores[1]} each)`;
        winner = "draw";
      }
