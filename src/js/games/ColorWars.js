...
export class ColorWars {
  ...
  _initializeGrid() {
    this.grid = Array(this.gridSize)
      ...
      .map(() =>
        Array(this.gridSize)
          .fill(null)
          .map(() => ({ owner: null, dots: 0, color: "neutral" })),
      );
    this.playerFirstMoves = { 1: false, 2: false };
    this.playerScores = { 1: 0, 2: 0 };
    this.gameEnded = false;
  ...
  _expandTerritory(row, col, depth = 0) {
    ...
    for (const [dr, dc] of directions) {
      ...
      if (!target || target.owner === null) {
        ...
        target.dots = total;
        target.color = currentPlayer === 1 ? "red" : "blue";
        this.playerScores[currentPlayer] += total;
        if (total === 4 && depth < this.maxExpansionDepth)
          cellsToExpand.push([nr, nc]);
      } else {
        target.dots++;
        this.playerScores[currentPlayer]++;
        if (target.dots === 4 && depth < this.maxExpansionDepth)
          cellsToExpand.push([nr, nc]);
      }
    }
  ...
  _checkWinCondition() {
    ...
    const canRed = this._canPlayerMove(1),
      canBlue = this._canPlayerMove(2);
    if (!canRed && !canBlue) {
      ...
      if (gridCell && gridCell.owner) {
        ...
        if (!this.playerFirstMoves[this.currentPlayer]) {
          cellEl.classList.add(
            this.currentPlayer === 1 ? "clickable-red" : "clickable-blue",
          );
        } else {
  ...
  getState() {
    return { currentPlayer: this.currentPlayer, gameActive: !this.gameEnded };
  }
  makeMove() {
    return { valid: false };
  }
